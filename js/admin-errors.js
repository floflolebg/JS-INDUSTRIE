...
// VERSION ERRORS - Utilise blog-api-errors.php pour voir les erreurs PHP
function getApiUrl(action, params = {}) {
    const currentPath = window.location.pathname;
    const currentDir = currentPath.substring(0, currentPath.lastIndexOf('/') + 1);
    const apiPath = currentDir + 'blog-api-errors.php'; // VERSION AVEC ERREURS

    const queryParams = new URLSearchParams({ action, ...params });
    console.log('🔍 API URL (avec erreurs):', `${apiPath}?${queryParams}`);
